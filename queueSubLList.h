...
template <typename T>
void queueSubLList<T>::pushAt(size_t index, const T &item)
{
    if (!head)
    {
        pushBack(item);
    ...
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < curr->sublist.getSize(); j++)
        {
            if ((k + j) == index)
            {
                ...
                resize(); // if(tail->sublist.isEmpty()) delete tail
            }
        }
        k += curr->sublist.getSize();
        curr = curr->next;
...
template <typename T>
T queueSubLList<T>::get(size_t index)
{
    Node<T> *curr = head;
    int k = 0;
    for (int i = 0; i < size; i++)
...
template <typename T>
T queueSubLList<T>::find(size_t key)
{
    Node<T> *curr = head;
    for (int i = 0; i < size; i++)
    {
        T item = curr->sublist.find(key);
        if(item)
            return item;
        curr = curr->next;
    }
